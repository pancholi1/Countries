...
function CountriesShow (){
    ...
    const pageVisited = pageNumber * usersPerPage
     ...
     const displayUsers = Array.isArray(countries) ? ( countries.slice(pageVisited, pageVisited + usersPerPage).map((country) => {
        return(<ul className= "ul-ordenamiento">

                    <li className="Cuadrado">
                            <Link to={`/nav/home/${country.id}`} className="linkCard">
                                <img src={country.img} className="Imagenes"></img>
                                <div className= "ordenamiento-name">  {country.name}</div>                     
                                <div className="ordenamiento-region">{country.region }</div>
                            </Link> 
                    </li>            
        </ul>) 
    })  ) : <span className= "notFound">The country you are looking for is not found... </span>;
     ...
     useEffect(()=>{
        ...
        return (    
            <div>   
                ...
                <div style={{padding: "20px"}} className="paginacion">
                    <ReactPaginate 
